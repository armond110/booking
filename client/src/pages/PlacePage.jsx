    ...
    return (
      <div className='absolute inset-0 bg-black text-white min-h-screen'>
        ...
        {place?.photos?.length > 0 && place.photos.map(photo => (
          <div key={photo} className="items-center mx-auto justify-center size-100">
  ...
  return (
    <div className="mt-8 bg-gray-50 -my-8 px-8   text-center">
      <h1 className="text-3xl">{place.title}</h1>
        ...
        <div className="relative  md:px-15 lg:px-40 ">
        <div className="md:grid md:gap-2 md:grid-cols-[2fr_2fr] md:rounded-3xl md:overflow-hidden ">
          <div className="">
            {place.photos?.[0] && (
              <div className="">
                <img onClick={() => setShowAllPhotos(true)} className="aspect-square object-cover cursor-pointer  rounded-2xl md:rounded-none  px-4 py-14 md:px-0 md:py-0" src={'http://localhost:3000/uploads/'+place.photos[0]} alt=""/>
              </div>
            )}
        </div>
